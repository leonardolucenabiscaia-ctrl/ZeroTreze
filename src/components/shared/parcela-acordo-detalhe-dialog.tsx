...
const FORMAS_PAGAMENTO_BAIXA: { value: NonNullable<ParcelaAcordo["formaPagamento"]>; label: string }[] = [
...
export interface BaixaManualAcordoInput {
  valor: number;
  formaPagamento: NonNullable<ParcelaAcordo["formaPagamento"]>;
  motivo: string;
}

/** Só faz sentido aplicar desconto ou dar baixa manual sobre parcelas de acordo ainda não
...
function descreverDesconto(desconto: DescontoParcelaAcordo): string {
  const partes: string[] = [];
...
export function ParcelaAcordoDetalheDialog({
  parcela,
  open,
  onOpenChange,
  ...
  onAplicarDesconto?: (parcelaAcordoId: string, desconto: DescontoParcelaAcordoInput) => Promise<void>;
  podeDarBaixa?: boolean;
  onDarBaixa?: (parcelaAcordoId: string, dados: BaixaManualAcordoInput) => Promise<void>;
}) {
  ...
  const [formaPagamentoBaixa, setFormaPagamentoBaixa] =
    React.useState<NonNullable<ParcelaAcordo["formaPagamento"]>>("dinheiro");
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            Parcela {parcela.numero} do acordo
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="flex flex-col gap-1.5">
                  <Label htmlFor="desconto-acordo-percentual" className="flex items-center gap-1.5 text-xs">
                    <Percent className="size-3.5 text-gold" />
                    Desconto de porcentagem
                  </Label>
                  <Input
                    id="desconto-acordo-percentual"
                    type="number"
                    min={0}
                    max={100}
                    step="0.1"
                ...
                <div className="flex flex-col gap-1.5">
                  <Label htmlFor="desconto-acordo-valor" className="flex items-center gap-1.5 text-xs">
                    <Tag className="size-3.5 text-gold" />
                    Desconto de valor (R$)
                  </Label>
                  <Input
                    id="desconto-acordo-valor"
                    type="number"
                    min={0}
                    step="0.01"
                    placeholder="0,00"
        ...
        {podeDarBaixa && podeReceberAcaoAdministrativa(parcela) && (
          <>
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="baixa-acordo-valor" className="flex items-center gap-1.5 text-xs">
                  <Banknote className="size-3.5 text-gold" />
                  Valor recebido (R$)
                </Label>
                <Input
                  id="baixa-acordo-valor"
                  type="number"
                  min={0}
                  step="0.01"
                  placeholder="0,00"
              ...
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="baixa-acordo-motivo" className="flex items-center gap-1.5 text-xs">
                  <MessageSquareText className="size-3.5 text-gold" />
                  Como foi recebido <span className="text-destructive">*</span>
                </Label>
                <Textarea
                  id="baixa-acordo-motivo"
